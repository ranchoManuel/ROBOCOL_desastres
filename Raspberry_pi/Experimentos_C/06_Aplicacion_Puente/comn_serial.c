...
void* leerEntradaSerial()
{
	while(1)
	{
		memset(buffReadSerial, 0, sizeof(buffReadSerial));
		/* We sleep the program(10000) because we have to wait for a full buffer */
		usleep(100000);
		nReadSerial=read(serial_fd, &buffReadSerial, BUFFSIZE);
		if(nReadSerial > 0)
		{
			enviarMensaje_a_Socket(buffReadSerial);
			fflush(stdout);
		}
	}
}
...
void initSerial(char *serialport, int baud)
{
	...
	/* if work, save current port settings */
	if(tcgetattr(serial_fd, &oldtio) < 0) closeWithError("serialport_init: Couldn't get term attributes");
