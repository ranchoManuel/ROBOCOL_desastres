...
void* tomarCamara()
{
	usleep(100); //dejar pasar una decima de segundo para que inicialize la pantalla
	while(continuar)
	{
		//8.1) Put the buffer in the incoming queue.
		if(ioctl(camara_fd[selectedCamera], VIDIOC_QBUF, &bufferinfo[selectedCamera]) < 0) closeWithError("VIDIOC_QBUF");

		//8.2) The buffer's waiting in the outgoing queue.
		if(ioctl(camara_fd[selectedCamera], VIDIOC_DQBUF, &bufferinfo[selectedCamera]) < 0) closeWithError("VIDIOC_QBUF");
...
void init_camaras(int cantPuertos, int argc, char* argv[])
{
	...
	selectedCamera=0;
	if(cantCamaras > MAX_CAMERA) closeWithError("Se necesita que MAX_CAMERA sea mayor a su valor actual");
	printf("Cantidad de camaras = %d\n", cantCamaras);
	...
	for(i=cantPuertos+1; i<argc; i++)
	{
		...
		if(camara_fd[esta] == -1)
		{
			sprintf(errMsj,"Unable to open camera:[%s]", argv[i]);
			closeWithError(errMsj);
		}
		printf("1) camera open: %s\n", argv[i]);

		//2) Retrieve the device’s capabilities
		if(ioctl(camara_fd[esta], VIDIOC_QUERYCAP, &cap) < 0)
		{
			sprintf(errMsj,"VIDIOC_QUERYCAP:[%s]", argv[i]);
			closeWithError(errMsj);
		}
		printf("2) The v4l2_capability structure if filled with information about the device: %s\n", argv[i]);
		...
		{
			sprintf(errMsj,"The device %s does not handle single-planar video capture.", argv[i]);
			closeWithError(errMsj);
		}
		printf("3) The device %s can handle single-planar video capture.\n", argv[i]);
		...
		{
			sprintf(errMsj,"VIDIOC_S_FMT:[%s]", argv[i]);
			closeWithError(errMsj);
		}
		printf("4) video format ok: %s\n", argv[i]);
		...
		if(ioctl(camara_fd[esta], VIDIOC_REQBUFS, &bufrequest) < 0) closeWithError("VIDIOC_REQBUFS");
		printf("5) video REQBUFS ok: %s\n", argv[i]);
		...
		if(ioctl(camara_fd[esta], VIDIOC_QUERYBUF, &bufferinfo[esta]) < 0) closeWithError("VIDIOC_QUERYBUF");
		printf("6) video QUERYBUF ok: %s\n", argv[i]);
		...
		if(buffer_start[esta] == MAP_FAILED) closeWithError("mmap");
		printf("7) mmap ok: %s\n", argv[i]);
		memset(buffer_start[esta], 0, bufferinfo[esta].length);

		//8) Activate streaming
		type = bufferinfo[esta].type;
		if(ioctl(camara_fd[esta], VIDIOC_STREAMON, &type) < 0) closeWithError("VIDIOC_STREAMON");
		printf("8) video STREAMON ok: %s\n", argv[i]);
	}
