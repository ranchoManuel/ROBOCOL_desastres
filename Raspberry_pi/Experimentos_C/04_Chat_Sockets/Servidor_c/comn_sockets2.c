...
int socket_desc2 ,socket_fd2 ,c2 ,read_size2;
...
void* leerEntrada_2()
{
	while(1)
  {
    ...
    socket_fd2 = accept(socket_desc2, (struct sockaddr *)&client2, (socklen_t*)&c2);
    if(socket_fd2 > 0)
		{
      ...
      while(1)
      {
				read_size2=recv(socket_fd2 ,buffRead2 ,sizeof(buffRead2) ,0);
...
void init_2(unsigned short puerto2)
{
    ...
    if(socket_desc2 == -1) closeWithError_2("Could not create socket(2)");
    ...
    //Bind
    if(bind(socket_desc2, (struct sockaddr *)&server2 , sizeof(server2)) < 0) closeWithError_2("bind failed. Error(2)");
    puts("bind done(2)");
...
{write(socket_fd2 ,texto2 ,strlen(texto2));}
